...
function renderPayPal() {
  ...
  if (!userData) {
    // 未登录用户：显示按钮并生成 bindCode
    document.querySelectorAll('.paypal-button-container').forEach(container => {
      container.innerHTML = '<div style="text-align:center;"><button class="btn btn-primary" id="tempSubscribeBtn">Subscribe Now</button></div>';
    });
    const tempBtn = document.getElementById('tempSubscribeBtn');
    if (tempBtn) {
      tempBtn.onclick = async () => {
        ...
        // 跳转到支付页面，默认 starter，用户需在支付页面选择套餐
        window.location.href = `https://paypal.taropai.com/?plan=starter&bindCode=${bindCode}`;
      };
    }
    return;
  }
...
function renderLanguage() {
  ...
  document.getElementById('planPremiumPrice').innerHTML = '$19.99<small>/month</small>';
  // 其他翻译保持不变...
}
...
function handleUrlParams() {
  ...
  if (action && email) {
    if (action === 'register') {
      ...
      document.getElementById('forgotPwdLink').click(); // 打开忘记密码弹窗
