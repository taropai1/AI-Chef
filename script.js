...
const PLANS = {
  free: { dailyLimit: 3, aiTrialLimit: 10, aiDailyLimit: 0 },
  starter: { dailyLimit: 10, aiTrialLimit: 10, aiDailyLimit: 0 },
  pro: { dailyLimit: 30, aiTrialLimit: 0, aiDailyLimit: 100 },
  premium: { dailyLimit: 80, aiTrialLimit: 0, aiDailyLimit: 200 },
  business: { dailyLimit: 300, aiTrialLimit: 0, aiDailyLimit: 300 }
};
...
async function prefetchVideosForCache() {
    try {
...
const translations = {
  en: {
    ...
    premiumName: 'Premium Family', businessName: 'Business Kitchen', starterDesc: '10 recipes/day · 10 AI questions (trial)',
    proDesc: '30 recipes/day · 100 AI questions/day', premiumDesc: '80 recipes/day shared · 200 AI questions/day · Family share',
    ...
    legalPrivacy1: 'We collect only the minimum necessary to provide the AI Chef service.\na) Account identifier (handled by a third‑party authentication service): When you sign up or log in, you provide your email address and a password to our trusted authentication provider (e.g., Auth0, Supabase, or Cloudflare Access). We never see your password. We receive only a unique user ID (derived from your email) for managing your subscription and usage rights. Your email is not stored in our own database (Cloudflare KV) – we store only an anonymous token or hash.\nb) Device fingerprint (for security & fraud prevention): We use fingerprintjs to collect a non‑unique, probabilistic identifier based on your browser settings, OS, screen resolution, fonts, and timezone. This helps prevent abuse (e.g., one person creating hundreds of free trials). It is NOT used to track you across different websites.\nc) Usage data (stored ONLY on your device): Recipes you generate, ingredients you enter, and AI chat history stay in your browser’s local storage. We never send them to our servers. They disappear when you clear browser data, log out, or refresh the page.\nd) Payment information (handled entirely by third parties): We never see your credit card or digital wallet details. Payments are processed by: PayPal (current); future: Apple Pay, Google Pay, WeChat Pay, Alipay, etc. You provide payment info directly to those processors – their own privacy policies apply.\ne) AI assistant queries: When you ask our AI Chef a question, your input is sent to DeepSeek API to generate a response. That conversation is not stored on our servers. DeepSeek may collect data according to its own policy, but we do not retain it.\nf) Email address (via Resend): We use Resend to send you essential service emails (verification, password reset) and, with your consent, occasional product updates. You can opt out of marketing emails at any time.',
    legalPrivacyUse: '2. Information Use', legalPrivacy2: 'We use your information only for the following purposes: \n- Authentication token / hashed email → to log you in, manage free trial limits, and subscription status via Cloudflare KV. \n- Device fingerprint → to prevent bots, fraud, and abusive free‑trial creation. \n- Local storage data → only for your convenience (so you can see recent recipes); we cannot access it. \n- Payment info → only seen by the payment processor to charge you for premium access. \n- Email address → for essential service emails and (with consent) occasional updates. We do NOT: \n- Sell your personal information. \n- Share your email with third‑party advertisers. \n- Store your recipes, ingredients, or AI conversations on our servers. \n- Have access to your password.',
    legalPrivacySecurity: '3. Data Security and Your Rights (GDPR / CCPA)', legalPrivacy3: 'Data storage locations: \n- Authentication data (email + encrypted password) is stored by the third‑party authentication provider (e.g., Auth0, Supabase). \n- Usage limits and subscription status are stored in Cloudflare KV (only anonymous tokens/hashes – not your plain email). \n- Frontend code is hosted on GitHub (static files, no user data). \n- Device fingerprint is calculated in your browser and shared as an identifier only. \n- Recipes and AI history stay in your browser’s local storage – we have no copy. \n- Payment info is stored by the payment processor (PayPal, etc.). Your rights (if you are in the EEA or California): - Access, correction, deletion, restriction, data portability, and opt‑out of “sale” (we do not sell personal information). To exercise any right, contact us at support@taropai.com. We will respond within 30 days. For California residents: Under CCPA, you have the right to know what personal information we collect, disclose, or sell. We collect only an anonymous token/hash + device fingerprint (no sale). For EU residents: We rely on legitimate interest (fraud prevention) for fingerprinting, and contract necessity for account/payment processing. You may object at any time.',
    legalPrivacyChanges: '4. Policy Changes', legalPrivacy4: 'If we make material changes to this policy, we will notify you via email (if you have an account) or by a notice on the website. The "Effective date" above will be updated accordingly.',
    legalPrivacyContact: '5. Contact Us', legalPrivacy5: 'For questions, complaints, or to exercise your privacy rights, contact us at: support@taropai.com',
    legalTermsTitle: 'Terms of Service', legalTermEffDate: 'Effective Date: 2026-01-01', legalTermsLicense: '1. License',
    ...
    planNotice: 'Secured by PayPal · Auto-renewal', featureStarter1: '10 recipes daily', featureStarter2: '10 AI questions (Gift)',
    featureStarter3: 'For personal use only', featureStarter4: 'Generate recipes from any ingredients', featurePro1: '30 recipes daily',
    featurePro2: '100 AI questions per day', featurePro3: 'All features unlocked', featurePro4: 'Advanced recipe customization',
    featurePremium1: '80 recipes daily', featurePremium2: '200 AI questions per day', featurePremium3: 'Up to 3 users sharing',
  ...
  'zh-CN': {
    ...
    proName: '高级订阅', premiumName: '家庭共享版', businessName: '商业厨房版', starterDesc: '10次/日 · 10次AI问答（试用）',
    proDesc: '30次/日 · 100次/日AI问答', premiumDesc: '80次/日家庭共享 · 200次/日AI问答', businessDesc: '300次/日 · 300次/日AI问答 · 商业用途',
    ...
    legalPrivacyTitle: '隐私政策', legalEffDate: '生效日期：2026年5月1日', legalPrivacyCollect: '1. 我们收集的信息',
    legalPrivacy1: '我们仅收集提供服务所必需的最少信息。\na) 账户标识（由第三方认证服务处理）：当您注册或登录时，您将电子邮件地址和密码提供给我们信任的认证提供商（例如 Auth0、Supabase 或 Cloudflare Access）。我们永远看不到您的密码。我们仅收到一个从您的电子邮件派生的唯一用户 ID，用于管理您的订阅。您的电子邮件不会存储在我们自己的数据库（Cloudflare KV）中 – 我们只存储匿名令牌或哈希值。\nb) 设备指纹（用于安全和防欺诈）：我们使用 fingerprintjs 收集基于您的浏览器设置、操作系统、屏幕分辨率、字体和时区的非唯一、概率性标识符。这有助于防止滥用（例如一人创建数百个免费试用）。它不会用于跨网站跟踪您。\nc) 使用数据（仅存储在您的设备上）：您生成的菜谱、输入的食材以及 AI 聊天记录都保留在您浏览器的本地存储中。我们从不将它们发送到我们的服务器。当您清除浏览器数据、退出登录或刷新页面时，这些数据会消失。\nd) 支付信息（完全由第三方处理）：我们永远不会看到您的信用卡或数字钱包详细信息。支付由以下方处理：PayPal（当前）；未来：Apple Pay、Google Pay、微信支付、支付宝等。您直接向这些处理商提供支付信息 – 适用他们自己的隐私政策。\ne) AI 助手查询：当您向我们的 AI 厨师提问时，您的输入会发送到 DeepSeek API 以生成回复。该对话不会存储在我们的服务器上。DeepSeek 可能会根据自己的政策收集数据，但我们不会保留。\nf) 电子邮件地址（通过 Resend）：我们使用 Resend 向您发送必要的服务邮件（验证、密码重置），并在您同意的情况下发送偶尔的产品更新。您可以随时取消订阅营销邮件。',
    legalPrivacyUse: '2. 信息使用', legalPrivacy2: '我们仅将您的信息用于以下目的：\n- 认证令牌 / 哈希邮件 → 用于登录、通过 Cloudflare KV 管理免费试用限制和订阅状态。\n- 设备指纹 → 用于防止机器人、欺诈和滥用免费试用。\n- 本地存储数据 → 仅为您方便（以便您查看最近的菜谱）；我们无法访问它们。\n- 支付信息 → 仅由支付处理商查看，用于向您收取高级访问费用。\n- 电子邮件地址 → 用于必要的服务邮件以及（经同意）偶尔的更新。我们不会：\n- 出售您的个人信息。\n- 将您的电子邮件分享给第三方广告商。\n- 在我们的服务器上存储您的菜谱、食材或 AI 对话。\n- 访问您的密码。',
    legalPrivacySecurity: '3. 数据安全与您的权利（GDPR / CCPA）', legalPrivacy3: '数据存储位置：\n- 认证数据（电子邮件 + 加密密码）由第三方认证提供商（例如 Auth0、Supabase）存储。\n- 使用限制和订阅状态存储在 Cloudflare KV 中（仅匿名令牌/哈希 – 不是您的明文电子邮件）。\n- 前端代码托管在 GitHub 上（静态文件，无用户数据）。\n- 设备指纹在您的浏览器中计算，仅作为标识符共享。\n- 菜谱和 AI 历史记录保留在您浏览器的本地存储中 – 我们没有副本。\n- 支付信息由支付处理商（PayPal 等）存储。您的权利（如果您在 EEA 或加利福尼亚州）：- 访问、更正、删除、限制、数据可携带权以及拒绝“出售”（我们不出售个人信息）。要行使任何权利，请通过 support@taropai.com 联系我们。我们将在 30 天内回复。对于加利福尼亚州居民：根据 CCPA，您有权知道我们收集、披露或出售哪些个人信息。我们仅收集匿名令牌/哈希 + 设备指纹（不出售）。对于欧盟居民：我们基于合法利益（防欺诈）使用指纹，并基于合同必要性处理账户/支付。您可以随时提出异议。',
    legalPrivacyChanges: '4. 政策变更', legalPrivacy4: '如果我们对此政策进行重大更改，我们将通过电子邮件（如果您有账户）或网站上的通知通知您。上述“生效日期”将相应更新。',
    legalPrivacyContact: '5. 联系我们', legalPrivacy5: '如有疑问、投诉或行使您的隐私权，请通过以下方式联系我们：support@taropai.com',
    legalTermsTitle: '服务条款', legalTermEffDate: '生效日期：2026-01-01', legalTermsLicense: '1. 许可',
    ...
    featureStarter1: '每日 10 次生成', featureStarter2: '10次AI问答（试用）', featureStarter3: '仅限个人使用', featureStarter4: '任意食材生成食谱',
    featurePro1: '每日 30 次生成', featurePro2: '每日100次AI问答', featurePro3: '解锁全部功能', featurePro4: '高级食谱定制',
    featurePremium1: '每日 80 次生成', featurePremium2: '每日200次AI问答', featurePremium3: '最多 3 人共享', featurePremium4: '共享用量池',
  ...
  es: {
    ...
    premiumName: 'Premium Family', businessName: 'Business Kitchen', starterDesc: '10 recetas/día · 10 preguntas IA (prueba)',
    proDesc: '30 recetas/día · 100 preguntas IA/día', premiumDesc: '80 recetas/día compartidas · 200 preguntas IA/día · Compartir en familia',
    businessDesc: '300 recetas/día · 300 preguntas IA/día · Uso comercial', finePrint: 'Al suscribirte aceptas nuestros ',
    ...
    emailTitle: 'Cambiar correo', legalPrivacyTitle: 'Política de privacidad', legalEffDate: 'Fecha de entrada en vigor: 2026-05-01',
    legalPrivacyCollect: '1. Información que Recopilamos', legalPrivacy1: 'Recopilamos solo lo mínimo necesario para ofrecer el servicio AI Chef.\na) Identificador de cuenta (gestionado por un servicio de autenticación externo): Cuando te registras o inicias sesión, proporcionas tu correo electrónico y una contraseña a nuestro proveedor de confianza (ej. Auth0, Supabase o Cloudflare Access). Nunca vemos tu contraseña. Solo recibimos un ID de usuario único derivado de tu correo para gestionar tu suscripción. Tu correo no se almacena en nuestra base de datos (Cloudflare KV) – solo guardamos un token anónimo o hash. \nb) Huella digital del dispositivo (para seguridad y prevención de fraude): Usamos fingerprintjs para recopilar un identificador probabilístico no único basado en la configuración de tu navegador, SO, resolución de pantalla, fuentes y zona horaria. Ayuda a prevenir abusos (ej. una persona creando cientos de pruebas gratuitas). NO se usa para rastrearte en diferentes sitios web. \nc) Datos de uso (almacenados SOLO en tu dispositivo): Las recetas que generas, ingredientes que ingresas y el historial del chat permanecen en el almacenamiento local de tu navegador. Nunca los enviamos a nuestros servidores. Desaparecen cuando borras los datos del navegador, cierras sesión o actualizas la página. \nd) Información de pago (gestionada completamente por terceros): Nunca vemos los detalles de tu tarjeta de crédito o billetera digital. Los pagos son procesados por: PayPal (actual); futuro: Apple Pay, Google Pay, WeChat Pay, Alipay, etc. Proporcionas la información de pago directamente a esos procesadores – aplican sus propias políticas de privacidad. \ne) Consultas al asistente de IA: Cuando haces una pregunta a nuestro AI Chef, tu entrada se envía a la API de DeepSeek para generar una respuesta. Esa conversación no se almacena en nuestros servidores. DeepSeek puede recopilar datos según su propia política, pero nosotros no los retenemos. \nf) Correo electrónico (a través de Resend): Usamos Resend para enviarte correos esenciales del servicio (verificación, restablecimiento de contraseña) y, con tu consentimiento, actualizaciones ocasionales del producto. Puedes darte de baja de los correos de marketing en cualquier momento.',
    legalPrivacyUse: '2. Uso de la Información', legalPrivacy2: 'Usamos tu información solo para los siguientes propósitos: \n- Token de autenticación / correo hasheado → para iniciar sesión, gestionar límites de prueba gratuita y estado de suscripción a través de Cloudflare KV. \n- Huella digital del dispositivo → para prevenir bots, fraudes y la creación abusiva de pruebas gratuitas. \n- Datos de almacenamiento local → solo para tu conveniencia (para que puedas ver recetas recientes); no podemos acceder a ellos. \n- Información de pago → solo vista por el procesador de pagos para cobrarte el acceso premium. \n- Correo electrónico → para correos esenciales del servicio y (con consentimiento) actualizaciones ocasionales. NO: \n- Vender tu información personal.\n- Compartir tu correo con anunciantes externos. \n- Almacenar tus recetas, ingredientes o conversaciones con IA en nuestros servidores. \n- Tener acceso a tu contraseña.',
    legalPrivacySecurity: '3. Seguridad de Datos y tus Derechos (GDPR / CCPA)', legalPrivacy3: 'Ubicaciones de almacenamiento de datos: \n- Los datos de autenticación (correo + contraseña cifrada) son almacenados por el proveedor de autenticación externo (ej. Auth0, Supabase). \n- Los límites de uso y el estado de suscripción se almacenan en Cloudflare KV (solo tokens anónimos/hashes – no tu correo en texto plano). \n- El código frontend está alojado en GitHub (archivos estáticos, sin datos de usuario). \n- La huella digital se calcula en tu navegador y se comparte solo como identificador. \n- Las recetas y el historial de IA permanecen en el almacenamiento local de tu navegador \n– no tenemos copia. \n- La información de pago es almacenada por el procesador de pagos (PayPal, etc.). Tus derechos (si estás en el EEE o California): \n- Acceso, corrección, eliminación, restricción, portabilidad de datos y exclusión de "venta" (no vendemos información personal). Para ejercer cualquier derecho, contáctanos en support@taropai.com. Responderemos en 30 días. Para residentes de California: Según CCPA, tienes derecho a saber qué información personal recopilamos, divulgamos o vendemos. Solo recopilamos un token anónimo/hash + huella digital (sin venta). Para residentes de la UE: Nos basamos en el interés legítimo (prevención de fraude) para la huella digital, y en la necesidad contractual para el procesamiento de cuentas/pagos. Puedes oponerte en cualquier momento.',
    legalPrivacyChanges: '4. Cambios en la Política', legalPrivacy4: 'Si realizamos cambios importantes en esta política, te notificaremos por correo electrónico (si tienes una cuenta) o mediante un aviso en el sitio web. La "Fecha de entrada en vigor" se actualizará en consecuencia.',
    legalPrivacyContact: '5. Contáctanos', legalPrivacy5: 'Para preguntas, quejas o para ejercer tus derechos de privacidad, contáctanos en: support@taropai.com',
    legalTermsTitle: 'Términos de servicio', legalTermEffDate: 'Fecha de vigencia: 2026-01-01', legalTermsLicense: '1. Licencia',
    ...
    featureStarter2: '10 preguntas IA (Regalo)', featureStarter3: 'Solo uso personal', featureStarter4: 'Genera recetas con cualquier ingrediente',
    featurePro1: '30 recetas diarias', featurePro2: '100 preguntas IA al día', featurePro3: 'Todas las funciones desbloqueadas',
    featurePro4: 'Personalización avanzada de recetas', featurePremium1: '80 recetas diarias', featurePremium2: '200 preguntas IA al día',
  ...
  fr: {
    ...
    premiumName: 'Premium Family', businessName: 'Business Kitchen', starterDesc: '10 recettes/jour · 10 questions IA (essai)',
    proDesc: '30 recettes/jour · 100 questions IA/jour', premiumDesc: '80 recettes/jour partagées · 200 questions IA/jour · Partage familial',
    businessDesc: '300 recettes/jour · 300 questions IA/jour · Usage commercial', finePrint: 'En vous abonnant, vous acceptez nos ',
    ...
    emailTitle: 'Changer d\'e-mail', legalPrivacyTitle: 'Politique de confidentialité', legalEffDate: 'Date d’entrée en vigueur : 2026-05-01',
    legalPrivacyCollect: '1. Informations que Nous Collectons', legalPrivacy1: 'Nous collectons uniquement le minimum nécessaire pour fournir le service AI Chef. \na) Identifiant de compte (géré par un service d’authentification tiers) : Lorsque vous vous inscrivez ou vous connectez, vous fournissez votre adresse e-mail et un mot de passe à notre fournisseur de confiance (ex. Auth0, Supabase ou Cloudflare Access). Nous ne voyons jamais votre mot de passe. Nous recevons uniquement un ID utilisateur unique (dérivé de votre e-mail) pour gérer votre abonnement. Votre e-mail n’est pas stocké dans notre propre base de données (Cloudflare KV) – nous stockons uniquement un jeton anonyme ou un hachage. \nb) Empreinte numérique de l’appareil (pour la sécurité et la prévention de la fraude) : Nous utilisons fingerprintjs pour collecter un identifiant probabiliste non unique basé sur les paramètres de votre navigateur, OS, résolution d’écran, polices et fuseau horaire. Cela aide à prévenir les abus (ex. une personne créant des centaines d’essais gratuits). Il n’est PAS utilisé pour vous suivre sur différents sites Web. \nc) Données d’utilisation (stockées UNIQUEMENT sur votre appareil) : Les recettes que vous générez, les ingrédients que vous saisissez et l’historique des discussions restent dans le stockage local de votre navigateur. Nous ne les envoyons jamais à nos serveurs. Ils disparaissent lorsque vous effacez les données de votre navigateur, vous déconnectez ou actualisez la page. \nd) Informations de paiement (traitées entièrement par des tiers) : Nous ne voyons jamais les détails de votre carte de crédit ou portefeuille numérique. Les paiements sont traités par : PayPal (actuel) ; futur : Apple Pay, Google Pay, WeChat Pay, Alipay, etc. Vous fournissez les informations de paiement directement à ces processeurs – leurs propres politiques de confidentialité s’appliquent. \ne) Requêtes à l’assistant IA : Lorsque vous posez une question à notre AI Chef, votre saisie est envoyée à l’API DeepSeek pour générer une réponse. Cette conversation n’est pas stockée sur nos serveurs. DeepSeek peut collecter des données conformément à sa propre politique, mais nous ne les conservons pas. \nf) Adresse e-mail (via Resend) : Nous utilisons Resend pour vous envoyer des e-mails essentiels du service (vérification, réinitialisation de mot de passe) et, avec votre consentement, des mises à jour occasionnelles du produit. Vous pouvez vous désabonner des e-mails marketing à tout moment.',
    legalPrivacyUse: '2. Utilisation des Informations', legalPrivacy2: 'Nous utilisons vos informations uniquement aux fins suivantes : \n- Jeton d’authentification / e-mail haché → pour vous connecter, gérer les limites d’essai gratuit et le statut de l’abonnement via Cloudflare KV. \n- Empreinte numérique de l’appareil → pour prévenir les bots, la fraude et la création abusive d’essais gratuits. \n- Données de stockage local → uniquement pour votre commodité (afin que vous puissiez voir les recettes récentes) ; nous n’y avons pas accès. \n- Informations de paiement → uniquement vues par le processeur de paiement pour vous facturer l’accès premium. \n- Adresse e-mail → pour les e-mails essentiels du service et (avec consentement) les mises à jour occasionnelles. Nous ne : \n- Vendons pas vos informations personnelles. \n- Partageons pas votre e-mail avec des annonceurs tiers. \n- Stockons pas vos recettes, ingrédients ou conversations IA sur nos serveurs. \n- Avons pas accès à votre mot de passe.',
    legalPrivacySecurity: '3. Sécurité des Données et Vos Droits (RGPD / CCPA)', legalPrivacy3: 'Emplacements de stockage des données : \n- Les données d’authentification (e-mail + mot de passe crypté) sont stockées par le fournisseur d’authentification tiers (ex. Auth0, Supabase). \n- Les limites d’utilisation et le statut de l’abonnement sont stockés dans Cloudflare KV (uniquement des jetons anonymes/hachages – pas votre e-mail en clair). \n- Le code frontend est hébergé sur GitHub (fichiers statiques, aucune donnée utilisateur). \n- L’empreinte numérique est calculée dans votre navigateur et partagée uniquement comme identifiant. \n- Les recettes et l’historique IA restent dans le stockage local de votre navigateur – nous n’avons pas de copie. \n- Les informations de paiement sont stockées par le processeur de paiement (PayPal, etc.). Vos droits (si vous êtes dans l’EEE ou en Californie) : - Accès, rectification, suppression, restriction, portabilité des données et opposition à la « vente » (nous ne vendons pas d’informations personnelles). Pour exercer un droit, contactez-nous à support@taropai.com. Nous répondrons dans les 30 jours. Pour les résidents californiens : En vertu du CCPA, vous avez le droit de savoir quelles informations personnelles nous collectons, divulguons ou vendons. Nous collectons uniquement un jeton anonyme/hachage + empreinte numérique (pas de vente). Pour les résidents de l’UE : Nous nous appuyons sur l’intérêt légitime (prévention de la fraude) pour l’empreinte numérique, et sur la nécessité contractuelle pour le traitement des comptes/paiements. Vous pouvez vous opposer à tout moment.',
    legalPrivacyChanges: '4. Modifications de la Politique', legalPrivacy4: 'Si nous apportons des modifications importantes à cette politique, nous vous en informerons par e-mail (si vous avez un compte) ou par un avis sur le site Web. La « Date d’entrée en vigueur » ci-dessus sera mise à jour en conséquence.',
    legalPrivacyContact: '5. Contactez-nous', legalPrivacy5: 'Pour toute question, réclamation ou pour exercer vos droits en matière de confidentialité, contactez-nous à : support@taropai.com',
    legalTermsTitle: 'Conditions d\'utilisation', legalTermEffDate: 'Date d\'entrée en vigueur : 2026-01-01', legalTermsLicense: '1. Licence',
    ...
    featureStarter2: '10 questions IA (Cadeau)', featureStarter3: 'Usage personnel uniquement', featureStarter4: 'Générez des recettes avec tous les ingrédients',
    featurePro1: '30 recettes par jour', featurePro2: '100 questions IA par jour', featurePro3: 'Toutes les fonctionnalités débloquées',
    featurePro4: 'Personnalisation avancée des recettes', featurePremium1: '80 recettes par jour', featurePremium2: '200 questions IA par jour',
    ...
    featureBusiness1: '300 générations par jour', featureBusiness2: '300 questions IA par jour', featureBusiness3: 'File de génération prioritaire',
  ...
  de: {
    ...
    premiumName: 'Premium Family', businessName: 'Business Kitchen', starterDesc: '10 Rezepte/Tag · 10 KI-Fragen (Test)',
    proDesc: '30 Rezepte/Tag · 100 KI-Fragen/Tag', premiumDesc: '80 Rezepte/Tag geteilt · 200 KI-Fragen/Tag · Familienfreigabe',
    businessDesc: '300 Rezepte/Tag · 300 KI-Fragen/Tag · Kommerzielle Nutzung', finePrint: 'Mit dem Abonnement stimmst du unseren ',
    ...
    emailTitle: 'E-Mail ändern', legalPrivacyTitle: 'Datenschutzerklärung', legalEffDate: 'Gültig ab: 2026-05-01',
    legalPrivacyCollect: '1. Informationen, die wir sammeln', legalPrivacy1: 'Wir sammeln nur das absolute Minimum, um den AI Chef Dienst bereitzustellen. \na) Kontoidentifikator (über einen Drittanbieter-Authentifizierungsdienst): Wenn Sie sich registrieren oder anmelden, geben Sie Ihre E-Mail-Adresse und ein Passwort bei unserem vertrauenswürdigen Anbieter (z.B. Auth0, Supabase oder Cloudflare Access) an. Wir sehen Ihr Passwort niemals. Wir erhalten nur eine eindeutige Benutzer-ID (abgeleitet von Ihrer E-Mail) zur Verwaltung Ihres Abonnements. Ihre E-Mail wird nicht in unserer eigenen Datenbank (Cloudflare KV) gespeichert – wir speichern nur ein anonymes Token oder einen Hash. \nb) Geräte-Fingerabdruck (für Sicherheit und Betrugsprävention): Wir verwenden fingerprintjs, um einen nicht-eindeutigen, probabilistischen Identifikator basierend auf Ihren Browsereinstellungen, Betriebssystem, Bildschirmauflösung, Schriftarten und Zeitzone zu sammeln. Dies hilft, Missbrauch zu verhindern (z.B. eine Person, die Hunderte von kostenlosen Testversionen erstellt). Es wird NICHT verwendet, um Sie über verschiedene Websites hinweg zu verfolgen. \nc) Nutzungsdaten (NUR auf Ihrem Gerät gespeichert): Von Ihnen generierte Rezepte, eingegebene Zutaten und der KI-Chatverlauf bleiben im lokalen Speicher Ihres Browsers. Wir senden sie niemals an unsere Server. Sie verschwinden, wenn Sie Ihre Browserdaten löschen, sich abmelden oder die Seite aktualisieren. \nd) Zahlungsinformationen (vollständig von Dritten verarbeitet): Wir sehen niemals Ihre Kreditkarten- oder digitalen Geldbörsendetails. Zahlungen werden verarbeitet von: PayPal (aktuell); zukünftig: Apple Pay, Google Pay, WeChat Pay, Alipay, usw. Sie geben die Zahlungsinformationen direkt an diese Anbieter – deren eigene Datenschutzrichtlinien gelten. \ne) KI-Assistent-Anfragen: Wenn Sie unserem AI Chef eine Frage stellen, wird Ihre Eingabe an die DeepSeek-API gesendet, um eine Antwort zu generieren. Dieses Gespräch wird nicht auf unseren Servern gespeichert. DeepSeek kann gemäß seiner eigenen Richtlinie Daten sammeln, wir behalten sie jedoch nicht. \nf) E-Mail-Adresse (über Resend): Wir verwenden Resend, um Ihnen wichtige Service-E-Mails (Verifizierung, Passwortzurücksetzung) und mit Ihrer Zustimmung gelegentliche Produktaktualisierungen zu senden. Sie können sich jederzeit von Marketing-E-Mails abmelden.',
    legalPrivacyUse: '2. Nutzung der Informationen', legalPrivacy2: 'Wir verwenden Ihre Informationen nur für folgende Zwecke: \n- Authentifizierungs-Token / gehashte E-Mail → um Sie anzumelden, kostenlose Testlimits und Abonnementstatus über Cloudflare KV zu verwalten. \n- Geräte-Fingerabdruck → um Bots, Betrug und missbräuchliche Erstellung kostenloser Testversionen zu verhindern. \n- Lokale Speicherdaten → nur für Ihre Bequemlichkeit (damit Sie aktuelle Rezepte sehen können); wir haben keinen Zugriff darauf. \n- Zahlungsinformationen → nur vom Zahlungsabwickler gesehen, um Ihnen den Premium-Zugang in Rechnung zu stellen. \n- E-Mail-Adresse → für wichtige Service-E-Mails und (mit Zustimmung) gelegentliche Aktualisierungen. Wir: \n- Verkaufen nicht Ihre persönlichen Daten. \n- Teilen Ihre E-Mail nicht mit Drittanbietern zu Werbezwecken. \n- Speichern nicht Ihre Rezepte, Zutaten oder KI-Gespräche auf unseren Servern. \n- Haben keinen Zugriff auf Ihr Passwort.',
    legalPrivacySecurity: '3. Datensicherheit und Ihre Rechte (DSGVO / CCPA)', legalPrivacy3: 'Speicherorte der Daten: \n- Authentifizierungsdaten (E-Mail + verschlüsseltes Passwort) werden vom Drittanbieter-Authentifizierungsdienst (z.B. Auth0, Supabase) gespeichert. \n- Nutzungslimits und Abonnementstatus werden in Cloudflare KV gespeichert (nur anonyme Tokens/Hashes – nicht Ihre Klartext-E-Mail). \n- Frontend-Code wird auf GitHub gehostet (statische Dateien, keine Benutzerdaten). \n- Der Geräte-Fingerabdruck wird in Ihrem Browser berechnet und nur als Identifikator geteilt. \n- Rezepte und KI-Verlauf bleiben im lokalen Speicher Ihres Browsers – wir haben keine Kopie. \n- Zahlungsinformationen werden vom Zahlungsabwickler gespeichert (PayPal, usw.). Ihre Rechte (wenn Sie im EWR oder in Kalifornien sind): - Zugriff, Berichtigung, Löschung, Einschränkung, Datenübertragbarkeit und Widerspruch gegen „Verkauf“ (wir verkaufen keine persönlichen Daten). Um Rechte auszuüben, kontaktieren Sie uns unter support@taropai.com. Wir antworten innerhalb von 30 Tagen. Für kalifornische Einwohner: Gemäß CCPA haben Sie das Recht zu erfahren, welche persönlichen Daten wir sammeln, offenlegen oder verkaufen. Wir sammeln nur ein anonymes Token/Hash + Geräte-Fingerabdruck (kein Verkauf). Für EU-Einwohner: Wir stützen uns auf berechtigtes Interesse (Betrugsprävention) für den Fingerabdruck und auf vertragliche Notwendigkeit für die Konto-/Zahlungsabwicklung. Sie können jederzeit Widerspruch einlegen.',
    legalPrivacyChanges: '4. Änderungen dieser Richtlinie', legalPrivacy4: 'Wenn wir wesentliche Änderungen an dieser Richtlinie vornehmen, benachrichtigen wir Sie per E-Mail (falls Sie ein Konto haben) oder durch einen Hinweis auf der Website. Das oben genannte „Gültig ab“-Datum wird entsprechend aktualisiert.',
    legalPrivacyContact: '5. Kontaktieren Sie uns', legalPrivacy5: 'Bei Fragen, Beschwerden oder zur Ausübung Ihrer Datenschutzrechte kontaktieren Sie uns unter: support@taropai.com',
    legalTermsTitle: 'Nutzungsbedingungen', legalTermEffDate: 'Gültig ab: 2026-01-01', legalTermsLicense: '1. Lizenz',
    ...
    featureStarter2: '10 KI-Fragen (Geschenk)', featureStarter3: 'Nur für den persönlichen Gebrauch', featureStarter4: 'Rezepte aus beliebigen Zutaten generieren',
    featurePro1: '30 Rezepte täglich', featurePro2: '100 KI-Fragen pro Tag', featurePro3: 'Alle Funktionen freigeschaltet',
    featurePro4: 'Erweiterte Rezeptanpassung', featurePremium1: '80 Rezepte täglich', featurePremium2: '200 KI-Fragen pro Tag',
    ...
    featureBusiness1: '300 tägliche Generierungen', featureBusiness2: '300 KI-Fragen pro Tag', featureBusiness3: 'Priorisierte Generierungswarteschlange',
  ...
  it: {
    ...
    premiumName: 'Premium Family', businessName: 'Business Kitchen', starterDesc: '10 ricette/giorno · 10 domande IA (prova)',
    proDesc: '30 ricette/giorno · 100 domande IA/giorno', premiumDesc: '80 ricette/giorno condivise · 200 domande IA/giorno · Condivisione familiare',
    businessDesc: '300 ricette/giorno · 300 domande IA/giorno · Uso commerciale', finePrint: 'Abbonandoti accetti i nostri ',
    ...
    emailTitle: 'Cambia email', legalPrivacyTitle: 'Informativa sulla privacy', legalEffDate: 'Data di entrata in vigore: 2026-05-01',
    legalPrivacyCollect: '1. Informazioni che Raccogliamo', legalPrivacy1: 'Raccogliamo solo il minimo necessario per fornire il servizio AI Chef. \na) Identificatore dell’account (gestito da un servizio di autenticazione di terze parti): Quando ti registri o accedi, fornisci il tuo indirizzo email e una password al nostro fornitore affidabile (es. Auth0, Supabase o Cloudflare Access). Non vediamo mai la tua password. Riceviamo solo un ID utente univoco (derivato dalla tua email) per gestire il tuo abbonamento. La tua email non viene memorizzata nel nostro database (Cloudflare KV) – memorizziamo solo un token anonimo o un hash. \nb) Impronta digitale del dispositivo (per sicurezza e prevenzione frodi): Utilizziamo fingerprintjs per raccogliere un identificatore probabilistico non unico basato sulle impostazioni del browser, sistema operativo, risoluzione dello schermo, font e fuso orario. Aiuta a prevenire abusi (es. una persona che crea centinaia di prove gratuite). NON viene utilizzato per tracciarti su diversi siti web. \nc) Dati di utilizzo (memorizzati SOLO sul tuo dispositivo): Le ricette che generi, gli ingredienti che inserisci e la cronologia delle chat rimangono nell’archiviazione locale del tuo browser. Non li inviamo mai ai nostri server. Scompaiono quando cancelli i dati del browser, esci o aggiorni la pagina. \nd) Informazioni di pagamento (gestite interamente da terze parti): Non vediamo mai i dettagli della tua carta di credito o portafoglio digitale. I pagamenti sono gestiti da: PayPal (attuale); futuro: Apple Pay, Google Pay, WeChat Pay, Alipay, ecc. Fornisci le informazioni di pagamento direttamente a questi processori – si applicano le loro politiche sulla privacy. \ne) Richieste all’assistente IA: Quando fai una domanda al nostro AI Chef, il tuo input viene inviato all’API di DeepSeek per generare una risposta. Questa conversazione non viene memorizzata sui nostri server. DeepSeek potrebbe raccogliere dati secondo la propria politica, ma noi non li conserviamo. f) Indirizzo email (tramite Resend): Utilizziamo Resend per inviarti email essenziali del servizio (verifica, reimpostazione password) e, con il tuo consenso, occasionali aggiornamenti del prodotto. Puoi disdire le email di marketing in qualsiasi momento.',
    legalPrivacyUse: '2. Utilizzo delle Informazioni', legalPrivacy2: 'Utilizziamo le tue informazioni solo per i seguenti scopi: \n- Token di autenticazione / email hashata → per effettuare il login, gestire i limiti della prova gratuita e lo stato dell’abbonamento tramite Cloudflare KV. \n- Impronta digitale del dispositivo → per prevenire bot, frodi e creazione abusiva di prove gratuite. \n- Dati di archiviazione locale → solo per tua comodità (in modo che tu possa vedere le ricette recenti); non possiamo accedervi. \n- Informazioni di pagamento → viste solo dal processore di pagamento per addebitarti l’accesso premium. \n- Indirizzo email → per email essenziali del servizio e (con consenso) aggiornamenti occasionali. Non: \n- Vendiamo le tue informazioni personali. \n- Condividiamo la tua email con inserzionisti terzi. \n- Memorizziamo le tue ricette, ingredienti o conversazioni IA sui nostri server. \n- Abbiamo accesso alla tua password.',
    legalPrivacySecurity: '3. Sicurezza dei Dati e i Tuoi Diritti (GDPR / CCPA)', legalPrivacy3: 'Luoghi di archiviazione dei dati: \n- I dati di autenticazione (email + password crittografata) sono memorizzati dal fornitore di autenticazione terzo (es. Auth0, Supabase). \n- I limiti di utilizzo e lo stato dell’abbonamento sono memorizzati in Cloudflare KV (solo token anonimi/hash – non la tua email in chiaro). \n- Il codice frontend è ospitato su GitHub (file statici, nessun dato utente). \n- L’impronta digitale viene calcolata nel tuo browser e condivisa solo come identificatore. \n- Le ricette e la cronologia IA rimangono nell’archiviazione locale del tuo browser – non ne abbiamo copia. \n- Le informazioni di pagamento sono memorizzate dal processore di pagamento (PayPal, ecc.). I tuoi diritti (se sei nel SEE o in California): - Accesso, rettifica, cancellazione, limitazione, portabilità dei dati e opt-out dalla “vendita” (non vendiamo informazioni personali). Per esercitare un diritto, contattaci a support@taropai.com. Risponderemo entro 30 giorni. Per i residenti in California: Ai sensi del CCPA, hai il diritto di sapere quali informazioni personali raccogliamo, divulghiamo o vendiamo. Raccogliamo solo un token anonimo/hash + impronta digitale (nessuna vendita). Per i residenti nell’UE: Facciamo affidamento sull’interesse legittimo (prevenzione frodi) per l’impronta digitale e sulla necessità contrattuale per l’elaborazione di account/pagamenti. Puoi opporti in qualsiasi momento.',
    legalPrivacyChanges: '4. Modifiche all’Informativa', legalPrivacy4: 'Se apportiamo modifiche sostanziali a questa informativa, ti notificheremo via email (se hai un account) o tramite un avviso sul sito web. La "Data di entrata in vigore" sopra verrà aggiornata di conseguenza.',
    legalPrivacyContact: '5. Contattaci', legalPrivacy5: 'Per domande, reclami o per esercitare i tuoi diritti sulla privacy, contattaci a: support@taropai.com',
    legalTermsTitle: 'Termini di servizio', legalTermEffDate: 'Data di entrata in vigore: 2026-01-01', legalTermsLicense: '1. Licenza',
    ...
    featureStarter2: '10 domande IA (Regalo)', featureStarter3: 'Solo uso personale', featureStarter4: 'Genera ricette con qualsiasi ingrediente',
    featurePro1: '30 ricette al giorno', featurePro2: '100 domande IA al giorno', featurePro3: 'Tutte le funzioni sbloccate',
    featurePro4: 'Personalizzazione avanzata delle ricette', featurePremium1: '80 ricette al giorno', featurePremium2: '200 domande IA al giorno',
  ...
  pt: {
    ...
    premiumName: 'Premium Family', businessName: 'Business Kitchen', starterDesc: '10 receitas/dia · 10 perguntas IA (avaliação)',
    proDesc: '30 receitas/dia · 100 perguntas IA/dia', premiumDesc: '80 receitas/dia compartilhadas · 200 perguntas IA/dia · Compartilhamento familiar',
    businessDesc: '300 receitas/dia · 300 perguntas IA/dia · Uso comercial', finePrint: 'Ao assinar, você concorda com nossos ',
    ...
    emailTitle: 'Alterar e-mail', legalPrivacyTitle: 'Política de privacidade', legalEffDate: 'Data de entrada em vigor: 2026-05-01',
    legalPrivacyCollect: '1. Informações que Coletamos', legalPrivacy1: 'Coletamos apenas o mínimo necessário para fornecer o serviço AI Chef. \na) Identificador de conta (gerenciado por um serviço de autenticação terceirizado): Quando você se registra ou faz login, você fornece seu endereço de e-mail e uma senha ao nosso provedor de confiança (ex., Auth0, Supabase ou Cloudflare Access). Nós nunca vemos sua senha. Recebemos apenas um ID de usuário único (derivado do seu e-mail) para gerenciar sua assinatura. Seu e-mail não é armazenado em nosso próprio banco de dados (Cloudflare KV) – armazenamos apenas um token anônimo ou hash. \nb) Impressão digital do dispositivo (para segurança e prevenção de fraudes): Usamos o fingerprintjs para coletar um identificador probabilístico não exclusivo com base nas configurações do seu navegador, SO, resolução de tela, fontes e fuso horário. Isso ajuda a prevenir abusos (ex., uma pessoa criando centenas de testes gratuitos). NÃO é usado para rastreá-lo em diferentes sites. \nc) Dados de uso (armazenados APENAS no seu dispositivo): As receitas que você gera, ingredientes que insere e o histórico de chat permanecem no armazenamento local do seu navegador. Nós nunca os enviamos para nossos servidores. Eles desaparecem quando você limpa os dados do navegador, sai da conta ou atualiza a página. \nd) Informações de pagamento (tratadas inteiramente por terceiros): Nunca vemos os detalhes do seu cartão de crédito ou carteira digital. Os pagamentos são processados por: PayPal (atual); futuro: Apple Pay, Google Pay, WeChat Pay, Alipay, etc. Você fornece as informações de pagamento diretamente a esses processadores – suas próprias políticas de privacidade se aplicam. \ne) Consultas ao assistente de IA: Quando você faz uma pergunta ao nosso AI Chef, sua entrada é enviada para a API da DeepSeek para gerar uma resposta. Essa conversa não é armazenada em nossos servidores. A DeepSeek pode coletar dados de acordo com sua própria política, mas não os retemos. \nf) Endereço de e-mail (via Resend): Usamos a Resend para enviar e-mails essenciais do serviço (verificação, redefinição de senha) e, com seu consentimento, atualizações ocasionais do produto. Você pode cancelar a assinatura de e-mails de marketing a qualquer momento.',
    legalPrivacyUse: '2. Uso das Informações', legalPrivacy2: 'Usamos suas informações apenas para as seguintes finalidades: \n- Token de autenticação / e-mail hash → para fazer login, gerenciar limites de teste gratuito e status de assinatura via Cloudflare KV. \n- Impressão digital do dispositivo → para evitar bots, fraudes e criação abusiva de testes gratuitos. \n- Dados de armazenamento local → apenas para sua conveniência (para que você possa ver receitas recentes); não podemos acessá-los. \n- Informações de pagamento → vistas apenas pelo processador de pagamento para cobrar seu acesso premium. \n- Endereço de e-mail → para e-mails essenciais do serviço e (com consentimento) atualizações ocasionais. Nós NÃO: \n- Vendemos suas informações pessoais. \n- Compartilhamos seu e-mail com anunciantes terceiros. \n- Armazenamos suas receitas, ingredientes ou conversas de IA em nossos servidores. \n- Temos acesso à sua senha.',
    legalPrivacySecurity: '3. Segurança dos Dados e Seus Direitos (GDPR / CCPA)', legalPrivacy3: 'Locais de armazenamento de dados: \n- Os dados de autenticação (e-mail + senha criptografada) são armazenados pelo provedor de autenticação terceirizado (ex., Auth0, Supabase). \n- Os limites de uso e o status da assinatura são armazenados no Cloudflare KV (apenas tokens anônimos/hashes – não seu e-mail em texto claro). \n- O código frontend está hospedado no GitHub (arquivos estáticos, sem dados do usuário). \n- A impressão digital é calculada no seu navegador e compartilhada apenas como identificador. \n- As receitas e o histórico de IA permanecem no armazenamento local do seu navegador – não temos cópia. \n- As informações de pagamento são armazenadas pelo processador de pagamento (PayPal, etc.). Seus direitos (se você estiver no EEE ou na Califórnia): - Acesso, correção, exclusão, restrição, portabilidade de dados e opt-out de “venda” (não vendemos informações pessoais). Para exercer qualquer direito, contate-nos em support@taropai.com . Responderemos em 30 dias. Para residentes da Califórnia: De acordo com a CCPA, você tem o direito de saber quais informações pessoais coletamos, divulgamos ou vendemos. Coletamos apenas um token anônimo/hash + impressão digital (sem venda). Para residentes da UE: Baseamo-nos no interesse legítimo (prevenção de fraudes) para a impressão digital e na necessidade contratual para o processamento de contas/pagamentos. Você pode se opor a qualquer momento.',
    legalPrivacyChanges: '4. Alterações nesta Política', legalPrivacy4: 'Se fizermos alterações materiais nesta política, notificaremos você por e-mail (se você tiver uma conta) ou por um aviso no site. A "Data de entrada em vigor" acima será atualizada de acordo.',
    legalPrivacyContact: '5. Contate-nos', legalPrivacy5: 'Para perguntas, reclamações ou para exercer seus direitos de privacidade, contate-nos em: support@taropai.com',
    legalTermsTitle: 'Termos de serviço', legalTermEffDate: 'Data de vigência: 2026-01-01', legalTermsLicense: '1. Licença',
    ...
    featureStarter2: '10 perguntas IA (Presente)', featureStarter3: 'Apenas para uso pessoal', featureStarter4: 'Gere receitas com qualquer ingrediente',
    featurePro1: '30 receitas por dia', featurePro2: '100 perguntas IA por dia', featurePro3: 'Todas as funções desbloqueadas',
    featurePro4: 'Personalização avançada de receitas', featurePremium1: '80 receitas por dia', featurePremium2: '200 perguntas IA por dia',
    ...
    featureBusiness1: '300 gerações por dia', featureBusiness2: '300 perguntas IA por dia', featureBusiness3: 'Fila de geração prioritária',
...
function renderVideoGrid(videos, player, titleEl, sourceEl, safeT) {
  ...
  card.addEventListener('click', () => {
    ...
    ytDiv.style.cssText = 'position:absolute; top:0; left:0; width:100%; height:100%;';
...
function renderProfile() {
    if (!userData) {
        ...
        if (aiTrialInfoEl) aiTrialInfoEl.style.display = 'none';
        return;
    }
...
async function showPage(pageId) {
    ...
    // 离开视频页时清除轮询定时器
    if (pageId !== 'page-video' && videoPollingTimer) {
        clearInterval(videoPollingTimer);
...
if (sendResetCodeBtn) sendResetCodeBtn.addEventListener('click', sendResetCode);

const sendEmailChangeCodeBtn = document.getElementById('sendEmailChangeCodeBtn');
if (sendEmailChangeCodeBtn) sendEmailChangeCodeBtn.addEventListener('click', sendEmailChangeCode);
...
function backToGenerator() {
  const recipeList = document.getElementById('recipeList');
  const hasRecipes = recipeList && recipeList.children.length > 0;
  switchGeneratorMode('recipe');
  if (hasRecipes) {
    document.getElementById('mainWrap')?.classList.add('top-fixed');
    document.getElementById('contentBlock')?.classList.add('show');
    document.getElementById('recipeContentWrapper')?.classList.add('show');
    document.getElementById('qaContent')?.classList.remove('show');
  } else {
    clearContentOnReset();
  }
...
function handleSend(e) {
  ...
  if (generatorMode === 'recipe') {
    ...
    if (!val) { showTipModal('Please enter a question.'); return; }
    sendLocked = true;
    const btn = document.getElementById('qaSendBtn');
    if (btn) btn.disabled = true;
    askQuestion();
