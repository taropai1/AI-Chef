...
const translations = {
  en: {
    ...
    legalPrivacy1: 'We collect only the minimum necessary to provide the AI Chef service. a) Account identifier (handled by a third‑party authentication service): When you sign up or log in, you provide your email address and a password to our trusted authentication provider (e.g., Auth0, Supabase, or Cloudflare Access). We never see your password. We receive only a unique user ID (derived from your email) for managing your subscription and usage rights. Your email is not stored in our own database (Cloudflare KV) – we store only an anonymous token or hash. b) Device fingerprint (for security & fraud prevention): We use fingerprintjs to collect a non‑unique, probabilistic identifier based on your browser settings, OS, screen resolution, fonts, and timezone. This helps prevent abuse (e.g., one person creating hundreds of free trials). It is NOT used to track you across different websites. c) Usage data (stored ONLY on your device): Recipes you generate, ingredients you enter, and AI chat history stay in your browser’s local storage. We never send them to our servers. They disappear when you clear browser data, log out, or refresh the page. d) Payment information (handled entirely by third parties): We never see your credit card or digital wallet details. Payments are processed by: PayPal (current); future: Apple Pay, Google Pay, WeChat Pay, Alipay, etc. You provide payment info directly to those processors – their own privacy policies apply. e) AI assistant queries: When you ask our AI Chef a question, your input is sent to DeepSeek API to generate a response. That conversation is not stored on our servers. DeepSeek may collect data according to its own policy, but we do not retain it. f) Email address (via Resend): We use Resend to send you essential service emails (verification, password reset) and, with your consent, occasional product updates. You can opt out of marketing emails at any time.',
    legalPrivacyUse: '2. Information Use', legalPrivacy2: 'We use your information only for the following purposes: - Authentication token / hashed email → to log you in, manage free trial limits, and subscription status via Cloudflare KV. - Device fingerprint → to prevent bots, fraud, and abusive free‑trial creation. - Local storage data → only for your convenience (so you can see recent recipes); we cannot access it. - Payment info → only seen by the payment processor to charge you for premium access. - Email address → for essential service emails and (with consent) occasional updates. We do NOT: - Sell your personal information. - Share your email with third‑party advertisers. - Store your recipes, ingredients, or AI conversations on our servers. - Have access to your password.',
    legalPrivacySecurity: '3. Data Security and Your Rights (GDPR / CCPA)', legalPrivacy3: 'Data storage locations: - Authentication data (email + encrypted password) is stored by the third‑party authentication provider (e.g., Auth0, Supabase). - Usage limits and subscription status are stored in Cloudflare KV (only anonymous tokens/hashes – not your plain email). - Frontend code is hosted on GitHub (static files, no user data). - Device fingerprint is calculated in your browser and shared as an identifier only. - Recipes and AI history stay in your browser’s local storage – we have no copy. - Payment info is stored by the payment processor (PayPal, etc.). Your rights (if you are in the EEA or California): - Access, correction, deletion, restriction, data portability, and opt‑out of “sale” (we do not sell personal information). To exercise any right, contact us at go@tarop.top. We will respond within 30 days. For California residents: Under CCPA, you have the right to know what personal information we collect, disclose, or sell. We collect only an anonymous token/hash + device fingerprint (no sale). For EU residents: We rely on legitimate interest (fraud prevention) for fingerprinting, and contract necessity for account/payment processing. You may object at any time.',
    ...
    legalPrivacyContact: '5. Contact Us', legalPrivacy5: 'For questions, complaints, or to exercise your privacy rights, contact us at: go@tarop.top',
  ...
  'zh-CN': {
    ...
    legalPrivacy1: '我们仅收集提供服务所必需的最少信息。a) 账户标识（由第三方认证服务处理）：当您注册或登录时，您将电子邮件地址和密码提供给我们信任的认证提供商（例如 Auth0、Supabase 或 Cloudflare Access）。我们永远看不到您的密码。我们仅收到一个从您的电子邮件派生的唯一用户 ID，用于管理您的订阅。您的电子邮件不会存储在我们自己的数据库（Cloudflare KV）中 – 我们只存储匿名令牌或哈希值。b) 设备指纹（用于安全和防欺诈）：我们使用 fingerprintjs 收集基于您的浏览器设置、操作系统、屏幕分辨率、字体和时区的非唯一、概率性标识符。这有助于防止滥用（例如一人创建数百个免费试用）。它不会用于跨网站跟踪您。c) 使用数据（仅存储在您的设备上）：您生成的菜谱、输入的食材以及 AI 聊天记录都保留在您浏览器的本地存储中。我们从不将它们发送到我们的服务器。当您清除浏览器数据、退出登录或刷新页面时，这些数据会消失。d) 支付信息（完全由第三方处理）：我们永远不会看到您的信用卡或数字钱包详细信息。支付由以下方处理：PayPal（当前）；未来：Apple Pay、Google Pay、微信支付、支付宝等。您直接向这些处理商提供支付信息 – 适用他们自己的隐私政策。e) AI 助手查询：当您向我们的 AI 厨师提问时，您的输入会发送到 DeepSeek API 以生成回复。该对话不会存储在我们的服务器上。DeepSeek 可能会根据自己的政策收集数据，但我们不会保留。f) 电子邮件地址（通过 Resend）：我们使用 Resend 向您发送必要的服务邮件（验证、密码重置），并在您同意的情况下发送偶尔的产品更新。您可以随时取消订阅营销邮件。',
    legalPrivacyUse: '2. 信息使用', legalPrivacy2: '我们仅将您的信息用于以下目的：- 认证令牌 / 哈希邮件 → 用于登录、通过 Cloudflare KV 管理免费试用限制和订阅状态。- 设备指纹 → 用于防止机器人、欺诈和滥用免费试用。- 本地存储数据 → 仅为您方便（以便您查看最近的菜谱）；我们无法访问它们。- 支付信息 → 仅由支付处理商查看，用于向您收取高级访问费用。- 电子邮件地址 → 用于必要的服务邮件以及（经同意）偶尔的更新。我们不会：- 出售您的个人信息。- 将您的电子邮件分享给第三方广告商。- 在我们的服务器上存储您的菜谱、食材或 AI 对话。- 访问您的密码。',
    legalPrivacySecurity: '3. 数据安全与您的权利（GDPR / CCPA）', legalPrivacy3: '数据存储位置：- 认证数据（电子邮件 + 加密密码）由第三方认证提供商（例如 Auth0、Supabase）存储。- 使用限制和订阅状态存储在 Cloudflare KV 中（仅匿名令牌/哈希 – 不是您的明文电子邮件）。- 前端代码托管在 GitHub 上（静态文件，无用户数据）。- 设备指纹在您的浏览器中计算，仅作为标识符共享。- 菜谱和 AI 历史记录保留在您浏览器的本地存储中 – 我们没有副本。- 支付信息由支付处理商（PayPal 等）存储。您的权利（如果您在 EEA 或加利福尼亚州）：- 访问、更正、删除、限制、数据可携带权以及拒绝“出售”（我们不出售个人信息）。要行使任何权利，请通过 go@tarop.top 联系我们。我们将在 30 天内回复。对于加利福尼亚州居民：根据 CCPA，您有权知道我们收集、披露或出售哪些个人信息。我们仅收集匿名令牌/哈希 + 设备指纹（不出售）。对于欧盟居民：我们基于合法利益（防欺诈）使用指纹，并基于合同必要性处理账户/支付。您可以随时提出异议。',
    legalPrivacyChanges: '4. 政策变更', legalPrivacy4: '如果我们对此政策进行重大更改，我们将通过电子邮件（如果您有账户）或网站上的通知通知您。上述“生效日期”将相应更新。',
    legalPrivacyContact: '5. 联系我们', legalPrivacy5: '如有疑问、投诉或行使您的隐私权，请通过以下方式联系我们：go@tarop.top',
  ...
  es: {
    ...
    legalPrivacyCollect: '1. Información que Recopilamos', legalPrivacy1: 'Recopilamos solo lo mínimo necesario para ofrecer el servicio AI Chef. a) Identificador de cuenta (gestionado por un servicio de autenticación externo): Cuando te registras o inicias sesión, proporcionas tu correo electrónico y una contraseña a nuestro proveedor de confianza (ej. Auth0, Supabase o Cloudflare Access). Nunca vemos tu contraseña. Solo recibimos un ID de usuario único derivado de tu correo para gestionar tu suscripción. Tu correo no se almacena en nuestra base de datos (Cloudflare KV) – solo guardamos un token anónimo o hash. b) Huella digital del dispositivo (para seguridad y prevención de fraude): Usamos fingerprintjs para recopilar un identificador probabilístico no único basado en la configuración de tu navegador, SO, resolución de pantalla, fuentes y zona horaria. Ayuda a prevenir abusos (ej. una persona creando cientos de pruebas gratuitas). NO se usa para rastrearte en diferentes sitios web. c) Datos de uso (almacenados SOLO en tu dispositivo): Las recetas que generas, ingredientes que ingresas y el historial del chat permanecen en el almacenamiento local de tu navegador. Nunca los enviamos a nuestros servidores. Desaparecen cuando borras los datos del navegador, cierras sesión o actualizas la página. d) Información de pago (gestionada completamente por terceros): Nunca vemos los detalles de tu tarjeta de crédito o billetera digital. Los pagos son procesados por: PayPal (actual); futuro: Apple Pay, Google Pay, WeChat Pay, Alipay, etc. Proporcionas la información de pago directamente a esos procesadores – aplican sus propias políticas de privacidad. e) Consultas al asistente de IA: Cuando haces una pregunta a nuestro AI Chef, tu entrada se envía a la API de DeepSeek para generar una respuesta. Esa conversación no se almacena en nuestros servidores. DeepSeek puede recopilar datos según su propia política, pero nosotros no los retenemos. f) Correo electrónico (a través de Resend): Usamos Resend para enviarte correos esenciales del servicio (verificación, restablecimiento de contraseña) y, con tu consentimiento, actualizaciones ocasionales del producto. Puedes darte de baja de los correos de marketing en cualquier momento.',
    legalPrivacyUse: '2. Uso de la Información', legalPrivacy2: 'Usamos tu información solo para los siguientes propósitos: - Token de autenticación / correo hasheado → para iniciar sesión, gestionar límites de prueba gratuita y estado de suscripción a través de Cloudflare KV. - Huella digital del dispositivo → para prevenir bots, fraudes y la creación abusiva de pruebas gratuitas. - Datos de almacenamiento local → solo para tu conveniencia (para que puedas ver recetas recientes); no podemos acceder a ellos. - Información de pago → solo vista por el procesador de pagos para cobrarte el acceso premium. - Correo electrónico → para correos esenciales del servicio y (con consentimiento) actualizaciones ocasionales. NO: - Vender tu información personal. - Compartir tu correo con anunciantes externos. - Almacenar tus recetas, ingredientes o conversaciones con IA en nuestros servidores. - Tener acceso a tu contraseña.',
    legalPrivacySecurity: '3. Seguridad de Datos y tus Derechos (GDPR / CCPA)', legalPrivacy3: 'Ubicaciones de almacenamiento de datos: - Los datos de autenticación (correo + contraseña cifrada) son almacenados por el proveedor de autenticación externo (ej. Auth0, Supabase). - Los límites de uso y el estado de suscripción se almacenan en Cloudflare KV (solo tokens anónimos/hashes – no tu correo en texto plano). - El código frontend está alojado en GitHub (archivos estáticos, sin datos de usuario). - La huella digital se calcula en tu navegador y se comparte solo como identificador. - Las recetas y el historial de IA permanecen en el almacenamiento local de tu navegador – no tenemos copia. - La información de pago es almacenada por el procesador de pagos (PayPal, etc.). Tus derechos (si estás en el EEE o California): - Acceso, corrección, eliminación, restricción, portabilidad de datos y exclusión de "venta" (no vendemos información personal). Para ejercer cualquier derecho, contáctanos en go@tarop.top. Responderemos en 30 días. Para residentes de California: Según CCPA, tienes derecho a saber qué información personal recopilamos, divulgamos o vendemos. Solo recopilamos un token anónimo/hash + huella digital (sin venta). Para residentes de la UE: Nos basamos en el interés legítimo (prevención de fraude) para la huella digital, y en la necesidad contractual para el procesamiento de cuentas/pagos. Puedes oponerte en cualquier momento.',
    ...
    legalPrivacyContact: '5. Contáctanos', legalPrivacy5: 'Para preguntas, quejas o para ejercer tus derechos de privacidad, contáctanos en: go@tarop.top',
  ...
  fr: {
    ...
    legalPrivacyCollect: '1. Informations que Nous Collectons', legalPrivacy1: 'Nous collectons uniquement le minimum nécessaire pour fournir le service AI Chef. a) Identifiant de compte (géré par un service d’authentification tiers) : Lorsque vous vous inscrivez ou vous connectez, vous fournissez votre adresse e-mail et un mot de passe à notre fournisseur de confiance (ex. Auth0, Supabase ou Cloudflare Access). Nous ne voyons jamais votre mot de passe. Nous recevons uniquement un ID utilisateur unique (dérivé de votre e-mail) pour gérer votre abonnement. Votre e-mail n’est pas stocké dans notre propre base de données (Cloudflare KV) – nous stockons uniquement un jeton anonyme ou un hachage. b) Empreinte numérique de l’appareil (pour la sécurité et la prévention de la fraude) : Nous utilisons fingerprintjs pour collecter un identifiant probabiliste non unique basé sur les paramètres de votre navigateur, OS, résolution d’écran, polices et fuseau horaire. Cela aide à prévenir les abus (ex. une personne créant des centaines d’essais gratuits). Il n’est PAS utilisé pour vous suivre sur différents sites Web. c) Données d’utilisation (stockées UNIQUEMENT sur votre appareil) : Les recettes que vous générez, les ingrédients que vous saisissez et l’historique des discussions restent dans le stockage local de votre navigateur. Nous ne les envoyons jamais à nos serveurs. Ils disparaissent lorsque vous effacez les données de votre navigateur, vous déconnectez ou actualisez la page. d) Informations de paiement (traitées entièrement par des tiers) : Nous ne voyons jamais les détails de votre carte de crédit ou portefeuille numérique. Les paiements sont traités par : PayPal (actuel) ; futur : Apple Pay, Google Pay, WeChat Pay, Alipay, etc. Vous fournissez les informations de paiement directement à ces processeurs – leurs propres politiques de confidentialité s’appliquent. e) Requêtes à l’assistant IA : Lorsque vous posez une question à notre AI Chef, votre saisie est envoyée à l’API DeepSeek pour générer une réponse. Cette conversation n’est pas stockée sur nos serveurs. DeepSeek peut collecter des données conformément à sa propre politique, mais nous ne les conservons pas. f) Adresse e-mail (via Resend) : Nous utilisons Resend pour vous envoyer des e-mails essentiels du service (vérification, réinitialisation de mot de passe) et, avec votre consentement, des mises à jour occasionnelles du produit. Vous pouvez vous désabonner des e-mails marketing à tout moment.',
    legalPrivacyUse: '2. Utilisation des Informations', legalPrivacy2: 'Nous utilisons vos informations uniquement aux fins suivantes : - Jeton d’authentification / e-mail haché → pour vous connecter, gérer les limites d’essai gratuit et le statut de l’abonnement via Cloudflare KV. - Empreinte numérique de l’appareil → pour prévenir les bots, la fraude et la création abusive d’essais gratuits. - Données de stockage local → uniquement pour votre commodité (afin que vous puissiez voir les recettes récentes) ; nous n’y avons pas accès. - Informations de paiement → uniquement vues par le processeur de paiement pour vous facturer l’accès premium. - Adresse e-mail → pour les e-mails essentiels du service et (avec consentement) les mises à jour occasionnelles. Nous ne : - Vendons pas vos informations personnelles. - Partageons pas votre e-mail avec des annonceurs tiers. - Stockons pas vos recettes, ingrédients ou conversations IA sur nos serveurs. - Avons pas accès à votre mot de passe.',
    legalPrivacySecurity: '3. Sécurité des Données et Vos Droits (RGPD / CCPA)', legalPrivacy3: 'Emplacements de stockage des données : - Les données d’authentification (e-mail + mot de passe crypté) sont stockées par le fournisseur d’authentification tiers (ex. Auth0, Supabase). - Les limites d’utilisation et le statut de l’abonnement sont stockés dans Cloudflare KV (uniquement des jetons anonymes/hachages – pas votre e-mail en clair). - Le code frontend est hébergé sur GitHub (fichiers statiques, aucune donnée utilisateur). - L’empreinte numérique est calculée dans votre navigateur et partagée uniquement comme identifiant. - Les recettes et l’historique IA restent dans le stockage local de votre navigateur – nous n’avons pas de copie. - Les informations de paiement sont stockées par le processeur de paiement (PayPal, etc.). Vos droits (si vous êtes dans l’EEE ou en Californie) : - Accès, rectification, suppression, restriction, portabilité des données et opposition à la « vente » (nous ne vendons pas d’informations personnelles). Pour exercer un droit, contactez-nous à go@tarop.top. Nous répondrons dans les 30 jours. Pour les résidents californiens : En vertu du CCPA, vous avez le droit de savoir quelles informations personnelles nous collectons, divulguons ou vendons. Nous collectons uniquement un jeton anonyme/hachage + empreinte numérique (pas de vente). Pour les résidents de l’UE : Nous nous appuyons sur l’intérêt légitime (prévention de la fraude) pour l’empreinte numérique, et sur la nécessité contractuelle pour le traitement des comptes/paiements. Vous pouvez vous opposer à tout moment.',
    ...
    legalPrivacyContact: '5. Contactez-nous', legalPrivacy5: 'Pour toute question, réclamation ou pour exercer vos droits en matière de confidentialité, contactez-nous à : go@tarop.top',
  ...
  de: {
    ...
    legalPrivacyCollect: '1. Informationen, die wir sammeln', legalPrivacy1: 'Wir sammeln nur das absolute Minimum, um den AI Chef Dienst bereitzustellen. a) Kontoidentifikator (über einen Drittanbieter-Authentifizierungsdienst): Wenn Sie sich registrieren oder anmelden, geben Sie Ihre E-Mail-Adresse und ein Passwort bei unserem vertrauenswürdigen Anbieter (z.B. Auth0, Supabase oder Cloudflare Access) an. Wir sehen Ihr Passwort niemals. Wir erhalten nur eine eindeutige Benutzer-ID (abgeleitet von Ihrer E-Mail) zur Verwaltung Ihres Abonnements. Ihre E-Mail wird nicht in unserer eigenen Datenbank (Cloudflare KV) gespeichert – wir speichern nur ein anonymes Token oder einen Hash. b) Geräte-Fingerabdruck (für Sicherheit und Betrugsprävention): Wir verwenden fingerprintjs, um einen nicht-eindeutigen, probabilistischen Identifikator basierend auf Ihren Browsereinstellungen, Betriebssystem, Bildschirmauflösung, Schriftarten und Zeitzone zu sammeln. Dies hilft, Missbrauch zu verhindern (z.B. eine Person, die Hunderte von kostenlosen Testversionen erstellt). Es wird NICHT verwendet, um Sie über verschiedene Websites hinweg zu verfolgen. c) Nutzungsdaten (NUR auf Ihrem Gerät gespeichert): Von Ihnen generierte Rezepte, eingegebene Zutaten und der KI-Chatverlauf bleiben im lokalen Speicher Ihres Browsers. Wir senden sie niemals an unsere Server. Sie verschwinden, wenn Sie Ihre Browserdaten löschen, sich abmelden oder die Seite aktualisieren. d) Zahlungsinformationen (vollständig von Dritten verarbeitet): Wir sehen niemals Ihre Kreditkarten- oder digitalen Geldbörsendetails. Zahlungen werden verarbeitet von: PayPal (aktuell); zukünftig: Apple Pay, Google Pay, WeChat Pay, Alipay, usw. Sie geben die Zahlungsinformationen direkt an diese Anbieter – deren eigene Datenschutzrichtlinien gelten. e) KI-Assistent-Anfragen: Wenn Sie unserem AI Chef eine Frage stellen, wird Ihre Eingabe an die DeepSeek-API gesendet, um eine Antwort zu generieren. Dieses Gespräch wird nicht auf unseren Servern gespeichert. DeepSeek kann gemäß seiner eigenen Richtlinie Daten sammeln, wir behalten sie jedoch nicht. f) E-Mail-Adresse (über Resend): Wir verwenden Resend, um Ihnen wichtige Service-E-Mails (Verifizierung, Passwortzurücksetzung) und mit Ihrer Zustimmung gelegentliche Produktaktualisierungen zu senden. Sie können sich jederzeit von Marketing-E-Mails abmelden.',
    legalPrivacyUse: '2. Nutzung der Informationen', legalPrivacy2: 'Wir verwenden Ihre Informationen nur für folgende Zwecke: - Authentifizierungs-Token / gehashte E-Mail → um Sie anzumelden, kostenlose Testlimits und Abonnementstatus über Cloudflare KV zu verwalten. - Geräte-Fingerabdruck → um Bots, Betrug und missbräuchliche Erstellung kostenloser Testversionen zu verhindern. - Lokale Speicherdaten → nur für Ihre Bequemlichkeit (damit Sie aktuelle Rezepte sehen können); wir haben keinen Zugriff darauf. - Zahlungsinformationen → nur vom Zahlungsabwickler gesehen, um Ihnen den Premium-Zugang in Rechnung zu stellen. - E-Mail-Adresse → für wichtige Service-E-Mails und (mit Zustimmung) gelegentliche Aktualisierungen. Wir: - Verkaufen nicht Ihre persönlichen Daten. - Teilen Ihre E-Mail nicht mit Drittanbietern zu Werbezwecken. - Speichern nicht Ihre Rezepte, Zutaten oder KI-Gespräche auf unseren Servern. - Haben keinen Zugriff auf Ihr Passwort.',
    legalPrivacySecurity: '3. Datensicherheit und Ihre Rechte (DSGVO / CCPA)', legalPrivacy3: 'Speicherorte der Daten: - Authentifizierungsdaten (E-Mail + verschlüsseltes Passwort) werden vom Drittanbieter-Authentifizierungsdienst (z.B. Auth0, Supabase) gespeichert. - Nutzungslimits und Abonnementstatus werden in Cloudflare KV gespeichert (nur anonyme Tokens/Hashes – nicht Ihre Klartext-E-Mail). - Frontend-Code wird auf GitHub gehostet (statische Dateien, keine Benutzerdaten). - Der Geräte-Fingerabdruck wird in Ihrem Browser berechnet und nur als Identifikator geteilt. - Rezepte und KI-Verlauf bleiben im lokalen Speicher Ihres Browsers – wir haben keine Kopie. - Zahlungsinformationen werden vom Zahlungsabwickler gespeichert (PayPal, usw.). Ihre Rechte (wenn Sie im EWR oder in Kalifornien sind): - Zugriff, Berichtigung, Löschung, Einschränkung, Datenübertragbarkeit und Widerspruch gegen „Verkauf“ (wir verkaufen keine persönlichen Daten). Um Rechte auszuüben, kontaktieren Sie uns unter go@tarop.top. Wir antworten innerhalb von 30 Tagen. Für kalifornische Einwohner: Gemäß CCPA haben Sie das Recht zu erfahren, welche persönlichen Daten wir sammeln, offenlegen oder verkaufen. Wir sammeln nur ein anonymes Token/Hash + Geräte-Fingerabdruck (kein Verkauf). Für EU-Einwohner: Wir stützen uns auf berechtigtes Interesse (Betrugsprävention) für den Fingerabdruck und auf vertragliche Notwendigkeit für die Konto-/Zahlungsabwicklung. Sie können jederzeit Widerspruch einlegen.',
    ...
    legalPrivacyContact: '5. Kontaktieren Sie uns', legalPrivacy5: 'Bei Fragen, Beschwerden oder zur Ausübung Ihrer Datenschutzrechte kontaktieren Sie uns unter: go@tarop.top',
  ...
  it: {
    ...
    legalPrivacyCollect: '1. Informazioni che Raccogliamo', legalPrivacy1: 'Raccogliamo solo il minimo necessario per fornire il servizio AI Chef. a) Identificatore dell’account (gestito da un servizio di autenticazione di terze parti): Quando ti registri o accedi, fornisci il tuo indirizzo email e una password al nostro fornitore affidabile (es. Auth0, Supabase o Cloudflare Access). Non vediamo mai la tua password. Riceviamo solo un ID utente univoco (derivato dalla tua email) per gestire il tuo abbonamento. La tua email non viene memorizzata nel nostro database (Cloudflare KV) – memorizziamo solo un token anonimo o un hash. b) Impronta digitale del dispositivo (per sicurezza e prevenzione frodi): Utilizziamo fingerprintjs per raccogliere un identificatore probabilistico non unico basato sulle impostazioni del browser, sistema operativo, risoluzione dello schermo, font e fuso orario. Aiuta a prevenire abusi (es. una persona che crea centinaia di prove gratuite). NON viene utilizzato per tracciarti su diversi siti web. c) Dati di utilizzo (memorizzati SOLO sul tuo dispositivo): Le ricette che generi, gli ingredienti che inserisci e la cronologia delle chat rimangono nell’archiviazione locale del tuo browser. Non li inviamo mai ai nostri server. Scompaiono quando cancelli i dati del browser, esci o aggiorni la pagina. d) Informazioni di pagamento (gestite interamente da terze parti): Non vediamo mai i dettagli della tua carta di credito o portafoglio digitale. I pagamenti sono gestiti da: PayPal (attuale); futuro: Apple Pay, Google Pay, WeChat Pay, Alipay, ecc. Fornisci le informazioni di pagamento direttamente a questi processori – si applicano le loro politiche sulla privacy. e) Richieste all’assistente IA: Quando fai una domanda al nostro AI Chef, il tuo input viene inviato all’API di DeepSeek per generare una risposta. Questa conversazione non viene memorizzata sui nostri server. DeepSeek potrebbe raccogliere dati secondo la propria politica, ma noi non li conserviamo. f) Indirizzo email (tramite Resend): Utilizziamo Resend per inviarti email essenziali del servizio (verifica, reimpostazione password) e, con il tuo consenso, occasionali aggiornamenti del prodotto. Puoi disdire le email di marketing in qualsiasi momento.',
    legalPrivacyUse: '2. Utilizzo delle Informazioni', legalPrivacy2: 'Utilizziamo le tue informazioni solo per i seguenti scopi: - Token di autenticazione / email hashata → per effettuare il login, gestire i limiti della prova gratuita e lo stato dell’abbonamento tramite Cloudflare KV. - Impronta digitale del dispositivo → per prevenire bot, frodi e creazione abusiva di prove gratuite. - Dati di archiviazione locale → solo per tua comodità (in modo che tu possa vedere le ricette recenti); non possiamo accedervi. - Informazioni di pagamento → viste solo dal processore di pagamento per addebitarti l’accesso premium. - Indirizzo email → per email essenziali del servizio e (con consenso) aggiornamenti occasionali. Non: - Vendiamo le tue informazioni personali. - Condividiamo la tua email con inserzionisti terzi. - Memorizziamo le tue ricette, ingredienti o conversazioni IA sui nostri server. - Abbiamo accesso alla tua password.',
    legalPrivacySecurity: '3. Sicurezza dei Dati e i Tuoi Diritti (GDPR / CCPA)', legalPrivacy3: 'Luoghi di archiviazione dei dati: - I dati di autenticazione (email + password crittografata) sono memorizzati dal fornitore di autenticazione terzo (es. Auth0, Supabase). - I limiti di utilizzo e lo stato dell’abbonamento sono memorizzati in Cloudflare KV (solo token anonimi/hash – non la tua email in chiaro). - Il codice frontend è ospitato su GitHub (file statici, nessun dato utente). - L’impronta digitale viene calcolata nel tuo browser e condivisa solo come identificatore. - Le ricette e la cronologia IA rimangono nell’archiviazione locale del tuo browser – non ne abbiamo copia. - Le informazioni di pagamento sono memorizzate dal processore di pagamento (PayPal, ecc.). I tuoi diritti (se sei nel SEE o in California): - Accesso, rettifica, cancellazione, limitazione, portabilità dei dati e opt-out dalla “vendita” (non vendiamo informazioni personali). Per esercitare un diritto, contattaci a go@tarop.top. Risponderemo entro 30 giorni. Per i residenti in California: Ai sensi del CCPA, hai il diritto di sapere quali informazioni personali raccogliamo, divulghiamo o vendiamo. Raccogliamo solo un token anonimo/hash + impronta digitale (nessuna vendita). Per i residenti nell’UE: Facciamo affidamento sull’interesse legittimo (prevenzione frodi) per l’impronta digitale e sulla necessità contrattuale per l’elaborazione di account/pagamenti. Puoi opporti in qualsiasi momento.',
    ...
    legalPrivacyContact: '5. Contattaci', legalPrivacy5: 'Per domande, reclami o per esercitare i tuoi diritti sulla privacy, contattaci a: go@tarop.top',
  ...
  pt: {
    ...
    legalPrivacyCollect: '1. Informações que Coletamos', legalPrivacy1: 'Coletamos apenas o mínimo necessário para fornecer o serviço AI Chef. a) Identificador de conta (gerenciado por um serviço de autenticação terceirizado): Quando você se registra ou faz login, você fornece seu endereço de e-mail e uma senha ao nosso provedor de confiança (ex., Auth0, Supabase ou Cloudflare Access). Nós nunca vemos sua senha. Recebemos apenas um ID de usuário único (derivado do seu e-mail) para gerenciar sua assinatura. Seu e-mail não é armazenado em nosso próprio banco de dados (Cloudflare KV) – armazenamos apenas um token anônimo ou hash. b) Impressão digital do dispositivo (para segurança e prevenção de fraudes): Usamos o fingerprintjs para coletar um identificador probabilístico não exclusivo com base nas configurações do seu navegador, SO, resolução de tela, fontes e fuso horário. Isso ajuda a prevenir abusos (ex., uma pessoa criando centenas de testes gratuitos). NÃO é usado para rastreá-lo em diferentes sites. c) Dados de uso (armazenados APENAS no seu dispositivo): As receitas que você gera, ingredientes que insere e o histórico de chat permanecem no armazenamento local do seu navegador. Nós nunca os enviamos para nossos servidores. Eles desaparecem quando você limpa os dados do navegador, sai da conta ou atualiza a página. d) Informações de pagamento (tratadas inteiramente por terceiros): Nunca vemos os detalhes do seu cartão de crédito ou carteira digital. Os pagamentos são processados por: PayPal (atual); futuro: Apple Pay, Google Pay, WeChat Pay, Alipay, etc. Você fornece as informações de pagamento diretamente a esses processadores – suas próprias políticas de privacidade se aplicam. e) Consultas ao assistente de IA: Quando você faz uma pergunta ao nosso AI Chef, sua entrada é enviada para a API da DeepSeek para gerar uma resposta. Essa conversa não é armazenada em nossos servidores. A DeepSeek pode coletar dados de acordo com sua própria política, mas não os retemos. f) Endereço de e-mail (via Resend): Usamos a Resend para enviar e-mails essenciais do serviço (verificação, redefinição de senha) e, com seu consentimento, atualizações ocasionais do produto. Você pode cancelar a assinatura de e-mails de marketing a qualquer momento.',
    legalPrivacyUse: '2. Uso das Informações', legalPrivacy2: 'Usamos suas informações apenas para as seguintes finalidades: - Token de autenticação / e-mail hash → para fazer login, gerenciar limites de teste gratuito e status de assinatura via Cloudflare KV. - Impressão digital do dispositivo → para evitar bots, fraudes e criação abusiva de testes gratuitos. - Dados de armazenamento local → apenas para sua conveniência (para que você possa ver receitas recentes); não podemos acessá-los. - Informações de pagamento → vistas apenas pelo processador de pagamento para cobrar seu acesso premium. - Endereço de e-mail → para e-mails essenciais do serviço e (com consentimento) atualizações ocasionais. Nós NÃO: - Vendemos suas informações pessoais. - Compartilhamos seu e-mail com anunciantes terceiros. - Armazenamos suas receitas, ingredientes ou conversas de IA em nossos servidores. - Temos acesso à sua senha.',
    legalPrivacySecurity: '3. Segurança dos Dados e Seus Direitos (GDPR / CCPA)', legalPrivacy3: 'Locais de armazenamento de dados: - Os dados de autenticação (e-mail + senha criptografada) são armazenados pelo provedor de autenticação terceirizado (ex., Auth0, Supabase). - Os limites de uso e o status da assinatura são armazenados no Cloudflare KV (apenas tokens anônimos/hashes – não seu e-mail em texto claro). - O código frontend está hospedado no GitHub (arquivos estáticos, sem dados do usuário). - A impressão digital é calculada no seu navegador e compartilhada apenas como identificador. - As receitas e o histórico de IA permanecem no armazenamento local do seu navegador – não temos cópia. - As informações de pagamento são armazenadas pelo processador de pagamento (PayPal, etc.). Seus direitos (se você estiver no EEE ou na Califórnia): - Acesso, correção, exclusão, restrição, portabilidade de dados e opt-out de “venda” (não vendemos informações pessoais). Para exercer qualquer direito, contate-nos em go@tarop.top. Responderemos em 30 dias. Para residentes da Califórnia: De acordo com a CCPA, você tem o direito de saber quais informações pessoais coletamos, divulgamos ou vendemos. Coletamos apenas um token anônimo/hash + impressão digital (sem venda). Para residentes da UE: Baseamo-nos no interesse legítimo (prevenção de fraudes) para a impressão digital e na necessidade contratual para o processamento de contas/pagamentos. Você pode se opor a qualquer momento.',
    ...
    legalPrivacyContact: '5. Contate-nos', legalPrivacy5: 'Para perguntas, reclamações ou para exercer seus direitos de privacidade, contate-nos em: go@tarop.top',
